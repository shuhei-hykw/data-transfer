...
src=(
    /mnt/raid/e96_bench
    /mnt/raid/e96_2025jan
    # /mnt/raid/software/param.git
    # /mnt/raid/subdata
)

host=login.cc.kek.jp
dest_disk=/group/had/sks/E96/JPARC2025Jan
dest_tape=/hsm/had/sks/E96/JPARC2025Jan
...
rsync="rsync -ahz --partial --whole-file"
...
do
	...
	if ( echo $s | grep -sq e96 && ls $s | grep -sq lock ); then
	    echo "$s -> skip"
	    echo
	    continue
	fi
	if echo $s | grep -sq e96; then
	    dest=$host:$dest_tape
	    echo "$s -> $dest"
	    $rsync --exclude="$s/run*" -e "$sshpass" $s $dest
	    tmp_include=/tmp/include-e96
